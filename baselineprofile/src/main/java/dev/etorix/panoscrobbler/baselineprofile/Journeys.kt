...
object Journeys {
    ...
    fun MacrobenchmarkScope.loginIfNeeded() {
        ...
        if (lastfmButton) {
            device.executeShellCommand("cmd notification allow_listener dev.etorix.panoscrobbler/dev.etorix.panoscrobbler.media.NLService")
            ...
            Thread.sleep(DELAY)
            device.findObject(By.res("button_stepper_open")).click()
            device.executeShellCommand("pm grant dev.etorix.panoscrobbler android.permission.POST_NOTIFICATIONS")
            device.waitForIdle()
        }
    }
