...
packagingRoot="$(mktemp -d "${TMPDIR:-/tmp}/pano-scrobbler-etd-package.XXXXXX")"
appDir="$packagingRoot/PanoScrobblerETD.AppDir"
libExecDir="$appDir/usr/libexec/$appNameWithoutSpaces"
distDir="$scriptDir/../dist"

# Read version code from version.txt
upstreamVerCode=$(cat "$scriptDir/../version.txt")
forkVerCode=$(cat "$scriptDir/../version-fork.txt")
verName="$((upstreamVerCode / 100)).$((upstreamVerCode % 100)).$forkVerCode"
...
ln -s "../../opt/$appNameWithoutSpaces/$appNameWithoutSpaces" "${debPkgDir}/usr/bin/${appNameWithoutSpaces}"
