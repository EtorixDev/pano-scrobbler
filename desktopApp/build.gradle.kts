...
val PACKAGE_VER_NAME = rootProject.extra["PACKAGE_VER_NAME"] as String
...
if (IS_WINDOWS) {
    tasks.register<Exec>("packageInno") {
        ...
        val iconFile = layout.projectDirectory.file("app-icons/windows/pano-scrobbler-etd.ico")
        ...
        outputs.file(layout.projectDirectory.file("../dist/$APP_NAME_NO_SPACES-$RESOURCES_DIR_NAME.exe"))
...
if (IS_WINDOWS) {
    tasks.register<Exec>("generateRc") {
        ...
        val rcOut = File(rcOutputDir, "$outputFileName.rc")
            val versionParts = PACKAGE_VER_NAME.split(".")
...
tasks.register<Sync>("packageNativeImage") {
    notCompatibleWithConfigurationCache("Native-image packaging uses Gradle script state.")
    ...
    val windowsTrayIconsDir = file("app-icons/windows")
    ...
    doLast {
        ...
        if (copyDesktopAndIcon) {
            ...
            val mainIconTarget = File(iconsOutputDir, "hicolor/scalable/apps/$APP_NAME_NO_SPACES.svg")
            mainIconTarget.parentFile.mkdirs()
            mainIconFile.copyTo(mainIconTarget, overwrite = true)
        ...
        if (IS_WINDOWS) {
            val trayIconsOutputDir = File(outputDir, "icons")
            trayIconsOutputDir.mkdirs()
            windowsTrayIconsDir.listFiles { icon ->
                icon.name.matches(Regex("pano-scrobbler-etd-(idle|scrobbling|error)-(black|white)\\.ico"))
            }?.forEach { icon ->
                icon.copyTo(File(trayIconsOutputDir, icon.name), overwrite = true)
            }
        }
